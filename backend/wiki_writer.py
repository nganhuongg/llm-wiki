...
def write_student_context(context: str) -> Path:
    page = config.STUDENT_DIR / "profile.md"
    body = f"""# Student Profile

## Current Context
{context.strip() or '_No context provided._'}

---
_Captured from the ingest flow._
"""
    page.write_text(body, encoding="utf-8")
    _append_changelog("updated student profile")
    return page


def write_study_guide(question: str, answer_md: str) -> Path:
    slug = _slug(question[:80] or "saved_answer")
    page = config.STUDY_GUIDES_DIR / f"{slug}.md"
    body = f"""# Study Guide: {question}

{answer_md}

---
_Saved from a useful answer._
"""
    page.write_text(body, encoding="utf-8")
    _append_changelog(f"saved study guide `{page.name}`")
    return page
...
## Student
{listing(config.STUDENT_DIR)}

## Study Guides
{listing(config.STUDY_GUIDES_DIR)}
"""
